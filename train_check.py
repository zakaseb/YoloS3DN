...
def main(config="config/config.py", experiment_name="default", world_size=1, local_rank=-1):
    ...
    dataloader_train = DataLoader(dataset_train, num_workers=cfg.data.num_workers,
                                  # num_workers is number of processes creating batches in parallel
                                  batch_size=cfg.data.batch_size, collate_fn=dataset_train.collate_fn,
                                  shuffle=local_rank < 0, drop_last=True,
                                  sampler=torch.utils.data.DistributedSampler(dataset_train, num_replicas=world_size,
                                                                              rank=local_rank,
                                                                              shuffle=True) if local_rank >= 0 else None)  # sampler determines how batches should be formed by specifiying sequences of iterable keys/indices to the objects
    dataloader_val = DataLoader(dataset_val, num_workers=cfg.data.num_workers,
                                batch_size=cfg.data.batch_size, collate_fn=dataset_val.collate_fn, shuffle=False,
                                drop_last=True)
    ...
    if 'training_func' in cfg.trainer:
        training_dection = PIPELINE_DICT[
            cfg.trainer.training_func]  # found in trainers.py in train_stereo_detection function
    else:
        raise KeyError
    ...
    for epoch_num in range(cfg.trainer.max_epochs):
        ...
        if is_evaluating and evaluate_detection is not None and cfg.trainer.test_iter > 0 and (
                epoch_num + 1) % cfg.trainer.test_iter == 0:
