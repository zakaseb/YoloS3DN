...
def process_train_val_file(cfg)-> Tuple[List[str], List[str]]:
    ...
    with open(train_file) as f: #opening and reading and returning copies with whitespace removed
...
def compute_dispairity_for_split(cfg,
    ...
    disp_dir = os.path.join(save_dir, 'disp') #saving the disp under the same directory for all preprocessing
    if not os.path.isdir(disp_dir):
        os.mkdir(disp_dir)

    if not use_point_cloud:
        stereo_matcher = cv2.StereoBM_create(192, 25) # stereo matching for camera-based
    ...
    for i, index_name in tqdm(enumerate(index_names)):
        ...
        if use_point_cloud:
            ...
            left_image  = denorm(image, cfg.data.augmentation.rgb_mean, cfg.data.augmentation.rgb_std) #denormalizing L&R images to calculate disparity
            right_image = denorm(image_3, cfg.data.augmentation.rgb_mean, cfg.data.augmentation.rgb_std)
            gray_image1 = cv2.cvtColor(left_image, cv2.COLOR_BGR2GRAY) #transform RGB to grayscale L&R images
            gray_image2 = cv2.cvtColor(right_image, cv2.COLOR_BGR2GRAY)

            disparity_left = stereo_matcher.compute(gray_image1, gray_image2) #calculating disparity of L wrt R image
            disparity_left[disparity_left < 0] = 0
            disparity_left = disparity_left.astype(np.uint16) #changing format
        ...
        disparity_left = skimage.measure.block_reduce(disparity_left, (4,4), np.max) #downsampling/pooling image 
        file_name = os.path.join(disp_dir, "P2%06d.png" % i) #adding newly created image from the downsampling to dir
        cv2.imwrite(file_name, disparity_left) #creating image

        disparity_right = skimage.measure.block_reduce(disparity_right, (4,4), np.max)
        file_name = os.path.join(disp_dir, "P3%06d.png" % i)
        cv2.imwrite(file_name, disparity_left) # not sure why left also here
