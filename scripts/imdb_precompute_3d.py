...
def process_train_val_file(cfg):
    train_file = cfg.data.train_split_file # the split of the dataset between training and eval/test 
...
def read_one_split(cfg, index_names, data_root_dir, output_dict, data_split = 'training', time_display_inter=100):
    ...
    if not os.path.isdir(save_dir):
        os.makedirs(save_dir) # saving data split in respective paths if they're not already saved

    if data_split == 'training':
        disp_dir = os.path.join(save_dir, 'disp') # adding training data to the disparity directory for disparity preprocessing
    ...
    for i, index_name in enumerate(index_names):
        ...
        data_frame = KittiData(data_root_dir, index_name, output_dict) #dataset specific, loading data from dataloader 
...
def main(config:str="config/config.py"):
    ...
    output_dict = {
                "calib": True, # make false if new backbone does not handle it 
                "image": True,
                "label": True,
                "velodyne": False,
            }
