...
class PSMCosineModule(nn.Module):
    ...
    def __init__(self, max_disp=192, downsample_scale=4, input_features=512):
        super(PSMCosineModule, self).__init__() #PSMCosine is cosine similarity module, the similarity is measured by the cosine of the angle of the two vectors in inner product
        self.max_disp = max_disp
        self.downsample_scale = downsample_scale
        self.depth_channel = int(self.max_disp / self.downsample_scale)
    ...
    def forward(self, left_features, right_features):
        ...
        for i in range(self.depth_channel): # for every channel, the cost is calculated by the mean of the multiplication of the right and left features 
            if i > 0 :
                 cost[:, i, :,i:]  = (left_features[:,:,:,i:] * right_features[:,:,:,:-i]).mean(dim=1) #nclear as to why i can be <0
            else:
                 cost[:, i, :, :]  = (left_features * right_features).mean(dim=1)
        cost = cost.contiguous() #elements of cost fct is to be stored in a contiguous (without spaces) together
        return cost
...
class DoublePSMCosineModule(PSMCosineModule):
    ...
    def forward(self, left_features, right_features):
        b, c, h, w = left_features.shape #b is for batch ?, c: channel ?, h: height, w: width
        base_grid_y, base_grid_x = make_grid(right_features.shape[2:]) #[h, w]
        base_grid_x = base_grid_x - 1.0 / right_features.shape[1] 
        shifted_grid = torch.stack([base_grid_y, base_grid_x], dim=-1).cuda().unsqueeze(0).repeat(b, 1, 1, 1) #right features are shifted horizontally over the left feature maps 
        ...
        return torch.cat([cost_1, cost_2], dim=1) # the two cost volumes are contacted together pixelwise as per the paper
