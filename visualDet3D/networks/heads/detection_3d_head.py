...
class AnchorBasedDetection3DHead(nn.Module):
    def __init__(self, num_features_in:int=1024,# number of features is 1024
                       num_classes:int=3, # 3 classes that the network predicts
                       num_regression_loss_terms=12,
                       preprocessed_path:str='',
                       anchors_cfg:EasyDict=EasyDict(), # configs from easydict which is a python lib
    ...
    def init_layers(self, num_features_in,
        ...
        self.cls_feature_extraction[-2].weight.data.fill_(0) #weights and biases of classification branch filled
        self.cls_feature_extraction[-2].bias.data.fill_(0)

        self.reg_feature_extraction = nn.Sequential( #regression branch feature extraction
        ...
        self.reg_feature_extraction[-2].weight.data.fill_(0) #weights and biases of regression branch filled
    ...
    def build_loss(self, focal_loss_gamma=0.0, balance_weight=[0], L1_regression_alpha=9, **kwargs): #this is to build and define specific loss functions needed 
        ...
        self.alpha_loss = nn.BCEWithLogitsLoss(reduction='none') # python package
...
class StereoHead(AnchorBasedDetection3DHead):
    def init_layers(self, num_features_in,
        ...
        self.cls_feature_extraction[-2].weight.data.fill_(0) #same as up
        self.cls_feature_extraction[-2].bias.data.fill_(0)
